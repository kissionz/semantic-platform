...
import { applyOntologyDefaults, defaultNumericSpec, inferNumericKind, inferObjectType, inferPropertyMeaning } from "../domain/modeling.js";
...
app.get("/api/bootstrap",async(request)=>{const storedDraft=store.ontology("DRAFT");const storedPublished=store.ontology("PUBLISHED");return {principal:request.principal,source:store.source(),tables:store.tables(),draft:applyOntologyDefaults(storedDraft||emptySnapshot((storedPublished?.version||0)+1)),published:storedPublished?applyOntologyDefaults(storedPublished):null,audits:store.audits(),users:request.principal?.role==="ADMIN"?store.users():[]};});
...
app.post<{Body:unknown}>("/api/ontology/from-table",async(request,reply)=>{
  ...
  const draft=applyOntologyDefaults(store.ontology("DRAFT")||emptySnapshot((store.ontology("PUBLISHED")?.version||0)+1));
...
app.put<{Body:unknown}>("/api/ontology/draft",async(request,reply)=>{requireRole(request.principal,["ADMIN","MODELER"]);const parsed=ontologySnapshotSchema.safeParse(request.body);if(!parsed.success)return reply.code(400).send({message:"本体草稿格式无效"});const snapshot=applyOntologyDefaults(parsed.data as OntologySnapshot);if(snapshot.status!=="DRAFT")return reply.code(400).send({message:"只能保存草稿"});const validation=[...validateSnapshot(snapshot),...validateCatalogBindings(snapshot)];store.saveOntology(snapshot);audit(request.principal!,"ONTOLOGY_DRAFT_SAVED",`v${snapshot.version}`,"SUCCESS",`${snapshot.objects.length} 个对象`);return {draft:snapshot,validation};});
app.post("/api/ontology/publish",async(request,reply)=>{requireRole(request.principal,["ADMIN","MODELER"]);const storedDraft=store.ontology("DRAFT");if(!storedDraft)return reply.code(409).send({message:"当前没有草稿"});const draft=applyOntologyDefaults(storedDraft);const validation=[...validateSnapshot(draft),...validateCatalogBindings(draft)];if(validation.some(issue=>issue.level==="ERROR")){audit(request.principal!,"ONTOLOGY_PUBLISHED",`v${draft.version}`,"REJECTED",`${validation.length} 项校验结果`);return reply.code(422).send({message:"草稿未通过校验",validation});}const published={...draft,status:"PUBLISHED" as const,publishedAt:new Date().toISOString(),objects:draft.objects.map(o=>({...o,status:"PUBLISHED" as const})),metrics:draft.metrics.map(m=>({...m,status:"PUBLISHED" as const})),relations:draft.relations.map(r=>({...r,status:"PUBLISHED" as const}))};store.saveOntology(published);audit(request.principal!,"ONTOLOGY_PUBLISHED",`v${published.version}`,"SUCCESS",`${published.objects.length} 个对象`);return {published};});

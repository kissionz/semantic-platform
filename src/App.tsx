...
import { inferCardinality, inferObjectType, inferRelationType, propertyMeaningLabels, relationTypeLabels, valueSearchableMeanings } from "./domain/modeling";
...
function EditPropertyDialog({ object, property, relations, onChange }: { object: OntologyObject; property: OntologyProperty; relations: OntologyRelation[]; onChange: (property: OntologyProperty) => void }) {
  ...
  const numericSpec = draft.numericSpec || { kind: "GENERAL" as const, unit: "", defaultAggregation: "SUM" as const, aggregationBehavior: "ADDITIVE" as const };
  ...
  const setMeaning = (meaning: OntologyProperty["meaning"]) => setDraft((current) => ({ ...current, meaning, valueSearchable: valueSearchableMeanings.has(meaning) ? current.valueSearchable : false, numericSpec: meaning === "NUMBER" ? current.numericSpec || { kind: "GENERAL", unit: "", defaultAggregation: "SUM", aggregationBehavior: "ADDITIVE" } : undefined }));
  ...
  return <Dialog open={open} onOpenChange={(_, data) => { setOpen(data.open); if (data.open) setDraft(property); }}>
    ...
    <DialogSurface className="wide-dialog"><DialogBody><DialogTitle>编辑属性</DialogTitle><DialogContent className="dialog-form property-editor">
      ...
      {draft.meaning === "NUMBER" && <div className="numeric-rules"><Field label="数值类型"><Select value={numericSpec.kind} onChange={(event) => { const kind = event.target.value as "GENERAL" | "CURRENCY" | "RATIO"; setNumeric({ kind, ...(kind === "RATIO" ? { defaultAggregation: "AVG", aggregationBehavior: "NON_ADDITIVE", unit: undefined, currency: undefined } : { currency: kind === "CURRENCY" ? numericSpec.currency : undefined }) }); }}><option value="GENERAL">数量 / 普通数值</option><option value="CURRENCY">货币</option><option value="RATIO">比例</option></Select></Field>{numericSpec.kind !== "RATIO" && <Field label="业务单位" required hint={numericSpec.kind === "CURRENCY" ? "如元、万元" : "如件、吨、小时"}><Input value={numericSpec.unit || ""} onChange={(_, data) => setNumeric({ unit: data.value })} /></Field>}{numericSpec.kind === "CURRENCY" && <Field label="币种" required hint="ISO 4217，如 CNY"><Input maxLength={3} value={numericSpec.currency || ""} onChange={(_, data) => setNumeric({ currency: data.value.toUpperCase() })} /></Field>}<Field label="默认聚合"><Select value={numericSpec.defaultAggregation} onChange={(event) => setNumeric({ defaultAggregation: event.target.value as "SUM" | "AVG" | "MIN" | "MAX" | "NONE" })}><option value="SUM" disabled={numericSpec.kind === "RATIO" || numericSpec.aggregationBehavior === "NON_ADDITIVE"}>SUM</option><option value="AVG">AVG</option><option value="MIN">MIN</option><option value="MAX">MAX</option><option value="NONE">NONE</option></Select></Field><Field label="可加性"><Select value={numericSpec.aggregationBehavior} disabled={numericSpec.kind === "RATIO"} onChange={(event) => { const aggregationBehavior = event.target.value as "ADDITIVE" | "SEMI_ADDITIVE" | "NON_ADDITIVE"; setNumeric({ aggregationBehavior, ...(aggregationBehavior === "NON_ADDITIVE" && numericSpec.defaultAggregation === "SUM" ? { defaultAggregation: "AVG" } : {}) }); }}><option value="ADDITIVE">可加</option><option value="SEMI_ADDITIVE">半可加</option><option value="NON_ADDITIVE">不可加</option></Select></Field></div>}

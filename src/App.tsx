...
import { defaultNumericSpec, hydrateNumericSpec, inferCardinality, inferObjectType, inferRelationType, propertyMeaningLabels, relationTypeLabels, valueSearchableMeanings } from "./domain/modeling";
...
function EditPropertyDialog({ object, property, relations, onChange }: { object: OntologyObject; property: OntologyProperty; relations: OntologyRelation[]; onChange: (property: OntologyProperty) => void }) {
  const [open, setOpen] = useState(false);
  const withDefaults = (item: OntologyProperty): OntologyProperty => item.meaning === "NUMBER" ? { ...item, numericSpec: hydrateNumericSpec(item.numericSpec) } : item;
  const [draft, setDraft] = useState(() => withDefaults(property));
  ...
  const valueSearchAllowed = valueSearchableMeanings.has(draft.meaning) && !structural && !draft.sensitive && draft.visibility === "ANALYTICAL";
  const numericSpec = hydrateNumericSpec(draft.numericSpec);
  const numericRequiredMissing = draft.meaning === "NUMBER" && ((numericSpec.kind === "GENERAL" && !numericSpec.unit?.trim()) || (numericSpec.kind === "CURRENCY" && (!numericSpec.unit?.trim() || !numericSpec.currency?.trim())));
  const set = <K extends keyof OntologyProperty>(key: K, value: OntologyProperty[K]) => setDraft((current) => ({ ...current, [key]: value }));
  const setMeaning = (meaning: OntologyProperty["meaning"]) => setDraft((current) => ({ ...current, meaning, valueSearchable: valueSearchableMeanings.has(meaning) ? current.valueSearchable : false, numericSpec: meaning === "NUMBER" ? hydrateNumericSpec(current.numericSpec) : undefined }));
  const setNumeric = (changes: Partial<NonNullable<OntologyProperty["numericSpec"]>>) => set("numericSpec", { ...numericSpec, ...changes });
  const save = () => {
    ...
    if (next.meaning === "NUMBER") next = { ...next, numericSpec: { ...(next.numericSpec || numericSpec), unit: next.numericSpec?.unit?.trim() || undefined, currency: next.numericSpec?.kind === "CURRENCY" ? next.numericSpec.currency?.trim().toUpperCase() || undefined : undefined } };
    if (next.sensitive || next.visibility !== "ANALYTICAL" || !valueSearchableMeanings.has(next.meaning)) next = { ...next, valueSearchable: false };
    onChange(next); setOpen(false);
  };
  return <Dialog open={open} onOpenChange={(_, data) => { setOpen(data.open); if (data.open) setDraft(withDefaults(property)); }}>
    ...
    <DialogSurface className="wide-dialog"><DialogBody><DialogTitle>编辑属性</DialogTitle><DialogContent className="dialog-form property-editor">
      ...
      <div className="property-editor-grid"><Field label="业务名称" required><Input value={draft.label} onChange={(_, data) => set("label", data.value)} /></Field><Field label="机器名称" required><Input value={draft.name} onChange={(_, data) => set("name", data.value)} /></Field><Field label="语义类型" required hint={structural ? "关系键的语义由关系约束" : undefined}><Select value={targetRelation ? "ID" : sourceMeaningLocked ? "ENTITY_REFERENCE" : draft.meaning} disabled={structural} onChange={(event) => setMeaning(event.target.value as OntologyProperty["meaning"])}>{Object.entries(propertyMeaningLabels).filter(([value]) => value !== "ID" || allowIdMeaning).map(([value, label]) => <option value={value} key={value}>{label}</option>)}</Select></Field><Field label="查询可见性"><Select value={structural ? "ANALYTICAL" : draft.visibility} disabled={structural} onChange={(event) => set("visibility", event.target.value as OntologyProperty["visibility"])}><option value="ANALYTICAL">可分析</option><option value="DETAIL_ONLY">仅明细</option><option value="HIDDEN">隐藏</option></Select></Field><Field label="绑定优先级"><Input type="number" min={0} max={1000} value={String(draft.bindingPriority)} onChange={(_, data) => set("bindingPriority", Number(data.value) || 0)} /></Field><Field label="同义词" hint="使用逗号分隔"><Input value={draft.synonyms.join("，")} onChange={(_, data) => set("synonyms", data.value.split(/[,，]/))} /></Field></div>
      <Field label="业务描述"><Textarea resize="vertical" value={draft.description} onChange={(_, data) => set("description", data.value)} /></Field>
      <div className="property-options" aria-label="属性规则"><Checkbox label="唯一值" checked={targetRelation || draft.meaning === "ID" ? true : draft.unique} disabled={Boolean(targetRelation) || draft.meaning === "ID"} onChange={(_, data) => set("unique", data.checked === true)} /><Checkbox label="敏感字段" checked={draft.sensitive} onChange={(_, data) => set("sensitive", data.checked === true)} /><Checkbox label="支持值检索" checked={valueSearchAllowed && draft.valueSearchable} disabled={!valueSearchAllowed} onChange={(_, data) => set("valueSearchable", data.checked === true)} /><Checkbox label="默认展示" checked={draft.defaultDisplay} onChange={(_, data) => set("defaultDisplay", data.checked === true)} /><Checkbox label="允许导出" checked={draft.exportable} onChange={(_, data) => set("exportable", data.checked === true)} /></div>
      {draft.meaning === "NUMBER" && <div className={`numeric-rules ${numericSpec.kind.toLowerCase()}`}><Field className="numeric-kind" label="数值类型"><Select value={numericSpec.kind} onChange={(event) => set("numericSpec", defaultNumericSpec(event.target.value as "GENERAL" | "CURRENCY" | "RATIO"))}><option value="GENERAL">数量 / 普通数值</option><option value="CURRENCY">货币</option><option value="RATIO">比例</option></Select></Field>{numericSpec.kind !== "RATIO" && <Field className="numeric-unit" label="业务单位" required hint={numericSpec.kind === "CURRENCY" ? "如元、万元" : "如件、吨、小时"}><Input value={numericSpec.unit || ""} onChange={(_, data) => setNumeric({ unit: data.value })} /></Field>}{numericSpec.kind === "CURRENCY" && <Field className="numeric-currency" label="币种" required hint="ISO 4217，如 CNY"><Input maxLength={3} value={numericSpec.currency || ""} onChange={(_, data) => setNumeric({ currency: data.value.toUpperCase() })} /></Field>}<Field className="numeric-aggregation" label="默认聚合"><Select value={numericSpec.defaultAggregation} onChange={(event) => setNumeric({ defaultAggregation: event.target.value as "SUM" | "AVG" | "MIN" | "MAX" | "NONE" })}><option value="SUM" disabled={numericSpec.kind === "RATIO" || numericSpec.aggregationBehavior === "NON_ADDITIVE"}>SUM</option><option value="AVG">AVG</option><option value="MIN">MIN</option><option value="MAX">MAX</option><option value="NONE">NONE</option></Select></Field><Field className="numeric-behavior" label="可加性"><Select value={numericSpec.aggregationBehavior} disabled={numericSpec.kind === "RATIO"} onChange={(event) => { const aggregationBehavior = event.target.value as "ADDITIVE" | "SEMI_ADDITIVE" | "NON_ADDITIVE"; setNumeric({ aggregationBehavior, ...(aggregationBehavior === "NON_ADDITIVE" && numericSpec.defaultAggregation === "SUM" ? { defaultAggregation: "AVG" } : {}) }); }}><option value="ADDITIVE">可加</option><option value="SEMI_ADDITIVE">半可加</option><option value="NON_ADDITIVE">不可加</option></Select></Field></div>}
    </DialogContent><DialogActions><Button appearance="secondary" onClick={() => setOpen(false)}>取消</Button><Button appearance="primary" onClick={save} disabled={!draft.label.trim() || !draft.name.trim() || numericRequiredMissing}>保存属性</Button></DialogActions></DialogBody></DialogSurface>
  </Dialog>;
}
...
function Ontology({ snapshot, setSnapshot, issues, onValidate, onSave, onPublish, tables, onModelTable }: { snapshot: OntologySnapshot; setSnapshot: (snapshot: OntologySnapshot) => void; issues: ValidationIssue[] | null; onValidate: () => void; onSave: () => void; onPublish: () => void; tables: Table[]; onModelTable: () => Promise<void> }) {
  ...
  const renderCatalog = () => {
    ...
    return relations.map((relation) => <button key={relation.id} onClick={() => setSelectedRelationId(relation.id)} className={`catalog-row ${selectedRelation?.id === relation.id ? "active" : ""}`}><span className="catalog-icon"><GitBranch size={17} /></span><span><strong>{relation.name}</strong><small>{relationTypeLabels[relation.type]}</small></span><CaretRight size={14} /></button>);
  };
